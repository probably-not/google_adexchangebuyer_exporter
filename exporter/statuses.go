...
type bidResponseWithoutBidsStatus struct {
	idx    int
	reason string
}

var bidResponsesWithoutBidsStatuses = map[string]bidResponseWithoutBidsStatus{
	"STATUS_UNSPECIFIED":                 {1, "Undefined status"},
	"RESPONSES_WITHOUT_BIDS":             {2, "The response had no bids"},
	"RESPONSES_WITHOUT_BIDS_FOR_ACCOUNT": {3, "The response had no bids for the specified account"},
	"RESPONSES_WITHOUT_BIDS_FOR_DEAL":    {4, "The response had no bids for the specified deal"},
}

type nonBillableWinningBidStatus struct {
	idx    int
	reason string
}

var nonBillableWinningBidsStatuses = map[string]nonBillableWinningBidStatus{
	"STATUS_UNSPECIFIED": {1, "Undefined status"},
	"AD_NOT_RENDERED":    {2, "The ad was not rendered by the publisher"},
	"INVALID_IMPRESSION": {3, "The impression won by the bid was invalid"},
	"FATAL_VAST_ERROR":   {4, "A fatal error was reported during video playback"},
	"LOST_IN_MEDIATION":  {5, "The ad was outplaced in the mediation waterfall"},
}

var calloutStatuses = map[int]string{
	1:  "Callout successful",
	2:  "Internal error",
	3:  "Unknown status (deprecated)",
	4:  "Unknown status",
	5:  "Out of quota",
	6:  "Predicted to be ignored",
	...
	23: "User identifier required but not present",
	24: "No placement in request",
	25: "Filtered by user consent settings",
	26: "Filtered by pretargeting quota",
	27: "Filtered due to missing a required SDK",
	28: "Blocked by publisher",
	29: "Bidding endpoint not configured",
	30: "Throttled - High response error rate",
	31: "Filtered due to buyer capabilities",
	32: "Third-party SDK rendering not supported",
	33: "Filtered due to CCPA publisher settings",
	34: "Filtered due to LGPD eligibility",
	35: "Encrypted signals required but missing",
}

var creativeStatuses = map[int]string{
	...
	182: "Creative filtered beacuse the publisher blacklisted the deal ID it targeted",
